...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic4(self, labels):
        ...
        mosaic_labels = []
        s = self.imgsz
        yc, xc = (int(random.uniform(-x, 2 * s + x)) for x in self.border)  # mosaic center x, y # 随机选择马赛克的中心位置
        for i in range(4):
            labels_patch = labels if i == 0 else labels["mix_labels"][i - 1]  # 处理基础图像和其他混合图像
            # Load image
            img = labels_patch["img"]  # 获取图像
            h, w = labels_patch.pop("resized_shape")  # 获取图像的高度和宽度

            # Place img in img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles # 创建一个空白基础图像
                x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)  定义大图像中的位置
                x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)  定义小图像中的位置
            elif i == 1:  # top right
...
class Format:
    ...
    def __init__(
        self,
        bbox_format="xywh",
        normalize=True,  # 是否将边界框的坐标归一化到 [0, 1] 范围内。这通常用于训练和推理时，确保边界框坐标在不同图像尺寸下的一致性。
        return_mask=False, #否返回实例分割的mask
        return_keypoint=False, # 是否返回关键点
        return_obb=False,  # 是否返回有方向的边界框,适用于需要处理旋转对象的任务。
        mask_ratio=4,  # 遮罩的下采样比率。当 return_mask 为 True 时，这个参数决定了mask的分辨率。
        mask_overlap=True, # 是否允许mask重叠。如果 True，则会允许在同一位置有多个mask重叠。
        batch_idx=True, # 是否保留批次索引,通常在处理批量数据时，保留批次索引有助于调试和分析。
        bgr=0.0,
    ):
        """
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    pre_transform = Compose(
        [
            Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic),  # 通过将四张图像随机组合为一张图像，使模型能够学习到不同大小和位置的目标物体
            CopyPaste(p=hyp.copy_paste),  # 通过从一张图像中复制目标物体并粘贴到另一张图像上来增加数据集的多样性，hyp.copy_paste 控制其应用概率
            RandomPerspective( # 对图像进行随机的透视变换，包括旋转、平移、缩放、剪切等，该变换帮助模型适应多角度的目标物体
                degrees=hyp.degrees,
    ...
    return Compose(
        [
            pre_transform,
            MixUp(dataset, pre_transform=pre_transform, p=hyp.mixup),  # 将两张图像混合为一张新图像，这有助于提高模型的泛化能力
            Albumentations(p=1.0),  # 通过Albumentations库进行高级增强操作
            RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),  # 对图像的色调、饱和度和亮度进行随机变化，增强颜色的多样性。
            RandomFlip(direction="vertical", p=hyp.flipud),  # 对图像进行垂直和水平翻转，增强模型对镜像图像的适应能力
            RandomFlip(direction="horizontal", p=hyp.fliplr, flip_idx=flip_idx),  # 控制翻转的概率
        ]
    )  # transforms
