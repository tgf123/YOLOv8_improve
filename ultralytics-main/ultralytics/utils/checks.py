...
from ultralytics.utils import ASSETS, AUTOINSTALL,IS_COLAB, IS_GIT_DIR, IS_JUPYTER, IS_KAGGLE, IS_PIP_PACKAGE
from ultralytics.utils import LINUX,LOGGER,MACOS, ONLINE, PYTHON_VERSION, ROOT, TORCHVISION_VERSION
from ultralytics.utils import USER_CONFIG_DIR,WINDOWS, Retry, SimpleNamespace, ThreadingLocked,TryExcept
from ultralytics.utils import clean_url,colorstr,downloads,emojis,is_github_action_running,url2file
...
def check_amp(model):
    ...
    def amp_allclose(m, im):  # 用于比较全精度（FP32）与自动混合精度（AMP）的推理结果
        """All close FP32 vs AMP results."""
        a = m(im, device=device, verbose=False)[0].boxes.data  # FP32 inference
        with autocast(enabled=True):
            b = m(im, device=device, verbose=False)[0].boxes.data  # AMP inference
        del m  #  推理结果的形状相同且在指定容差范围内0.5，说明 AMP 功能正常
    ...
    try:
        ...
        assert amp_allclose(YOLO("yolov8n.pt"), im)  # 查 AMP 推理结果是否与 FP32 相似。如果检查通过，输出日志提示“通过”。
