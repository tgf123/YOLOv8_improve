...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        ...
        # FP32 EMA 通过 deepcopy 创建了模型的副本，用于计算 EMA。使用 de_parallel 函数移除分布式并行的包装，
        # 确保 EMA 的计算不受分布式训练的影响,设置为eval模式，以确保在计算EMA时不会有dropout或batch normalization 的影响。
        self.ema = deepcopy(de_parallel(model)).eval()
        self.updates = updates  # number of EMA updates 记录EMA更新的次数
        # 用于计算 EMA 的衰减率，衰减率随时间的变化而变化，以帮助模型在早期训练阶段进行更平滑的更新
        self.decay = lambda x: decay * (1 - math.exp(-x / tau))
        for p in self.ema.parameters():
            p.requires_grad_(False) # 禁用 EMA 模型参数的梯度计算。EMA 模型参数不参与梯度计算，因为 EMA 只用于平滑模型参数
        self.enabled = True  # 用于启用或禁用 EMA。默认为 True
