...
class Model(nn.Module):
    ...
    Methods:
        ...
        __call__: 该方法将类实例变为可调用对象，实际是predict方法的别名，使得可以像调用函数一样使用模型实例进行预测。
        _new: 根据给定的配置文件（.yaml）初始化一个新的模型实例。
        _load: 从指定的检查点（.pt文件）加载模型。
        _check_is_pytorch_model: 检查并确保模型是 PyTorch 格式的模型。
        reset_weights: 将模型的权重重置为初始状态，通常用于重新训练或测试。
        load: 从指定文件加载模型的权重。
        save: 将当前模型状态保存到指定文件。
        info: 记录或返回模型的详细信息，如结构和配置。
        fuse: 融合 Conv2d 和 BatchNorm2d 层以优化推理性能，减少推理时的计算开销。
        predict: 进行目标检测预测，通常输入图像或视频，输出预测结果。
        track: 执行目标跟踪任务。
        val: 在数据集上验证模型的性能。
        benchmark: 对模型进行基准测试，评估其在不同导出格式下的性能。
        export: 将模型导出到不同的格式（如 ONNX、TorchScript 等）。
        train: 使用指定数据集训练模型。
        tune: 进行超参数调优，自动寻找最佳训练参数。
        _apply: 对模型的张量（tensors）应用某个函数。
        add_callback: 为某个事件添加回调函数。
        clear_callback: 清除某个事件的所有回调函数。
    ...
    def __init__(
        ...
        """
        super().__init__()
        # 方法获取默认的回调函数列表,这些回调将会在模型训练、验证等过程中的特定阶段被调用，执行预定义的任务。
        ...
        self.ckpt_path = None
        self.overrides = {}  # overrides for trainer object 是个字典格式存储配置参数
        self.metrics = None  # validation/training metrics  存储最新的训练或验证的评估指标
        self.session = None  # HUB session
        self.task = task  # task type
        model = str(model).strip()  # 对传入的对象将其转换为字符串并去除两端的空格

        # Check if Ultralytics HUB model from https://hub.ultralytics.com
        if self.is_hub_model(model): # 检查提供的 model 是否来自 Ultralytics HUB
        ...
        elif self.is_triton_model(model): #检查提供的 model 是否来自 Triton Server
            self.model_name = self.model = model
            return

        # Load or create new YOLO model
        if Path(model).suffix in {".yaml", ".yml"}:  # 根据给定的配置文件（.yaml）初始化一个新的模型实例
            self._new(model, task=task, verbose=verbose)
        else:
            self._load(model, task=task)  # 从指定的检查点（.pt文件）加载模型

    def __call__(  # 该方法将类实例变为可调用对象，实际是predict方法的别名，使得可以像调用函数一样使用模型实例进行预测。
        self,
    ...
    def _load(self, weights: str, task=None) -> None:
        ...
        Examples:
            ...
            从指定的检查点（.pt文件）加载模型
        """
        # 处理来自网络的权重文件。支持多种协议（如 https: // 、http: // 等）。如果传入的 weights参数是一个URL，代码会通过checks.check_file方法下载该权重文件，并将其保存在指定的目录（SETTINGS["weights_dir"]）中，然后返回下载后的本地文件路径。
        if weights.lower().startswith(("https://", "http://", "rtsp://", "rtmp://", "tcp://")):
            weights = checks.check_file(weights, download_dir=SETTINGS["weights_dir"])  # download and return local file
        weights = checks.check_model_file_from_stem(weights)  # add suffix, i.e. yolov8n -> yolov8n.pt为模型名称添加正确的后缀

        if Path(weights).suffix == ".pt":
            self.model, self.ckpt = attempt_load_one_weight(weights)  # 加载训练好的模型权重，返回模型结构、权重和训练的超参数等信息
            self.task = self.model.args["task"]  # 从模型的参数中提取任务类型 task
    ...
    def fuse(self):
        ...
        Examples:
            ...
            融合 Conv2d 和 BatchNorm2d 层以优化推理性能，减少推理时的计算开销
        """
        self._check_is_pytorch_model()
        self.model.fuse()
    ...
    def train(
        ...
        **kwargs,
        ...
        该方法支持自定义训练器或默认的训练方式。它能够处理从检查点恢复训练、与 Ultralytics HUB 集成、
        以及在训练完成后更新模型和配置的场景。

        参数:
            trainer (BaseTrainer | None): 自定义的训练器实例。如果为 None，则使用默认的训练器。
            **kwargs (Any): 训练配置的关键字参数，例如:
                data (str): 数据集配置文件的路径。
                epochs (int): 训练轮数。
                batch_size (int): 训练的批次大小。
                imgsz (int): 输入图像的尺寸。
                device (str): 训练所用设备（如 'cuda'、'cpu'）。
                workers (int): 数据加载的工作线程数。
                optimizer (str): 用于训练的优化器。
                lr0 (float): 初始学习率。
                patience (int): 早停策略中，无改善时等待的轮数。

        返回:
            (Dict | None): 如果训练成功，返回训练的度量指标；否则返回 None。

        抛出:
            AssertionError: 如果模型不是 PyTorch 模型。
            PermissionError: 如果与 HUB 会话相关的权限问题。
        ...
        Examples:
            ...
            使用指定数据集训练模型
        """
        self._check_is_pytorch_model()  # 检查当前模型是否为 PyTorch 模型
        if hasattr(self.session, "model") and self.session.model.id:  # Ultralytics HUB session with loaded model
            if any(kwargs):
                LOGGER.warning("WARNING ⚠️ using HUB training arguments, ignoring local training arguments.")
            kwargs = self.session.train_args  # overwrite kwargs

        checks.check_pip_update_available()  # 检查是否需要更新包
        ...
        args = {**overrides, **custom, **kwargs, "mode": "train"}  # 合并所有参数
        if args.get("resume"):
            args["resume"] = self.ckpt_path
        # 支持通过自定义 trainer 实例进行训练。如果未指定 trainer，将使用默认的训练器。
        self.trainer = (trainer or self._smart_load("trainer"))(overrides=args, _callbacks=self.callbacks)
        if not args.get("resume"):  # manually set model only if not resuming
            self.trainer.model = self.trainer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml)
            self.model = self.trainer.model  # 将 self.trainer 中获取到的模型实例赋值给 self.model

        self.trainer.hub_session = self.session  # attach optional HUB session
        self.trainer.train()  # 开始模型训练
    ...
    def _smart_load(self, key: str):
        ...
        """
        try:  # 根据模型的任务（task）动态加载对应的模块（如 model、trainer、validator 或 predictor）。它通过 task_map 属性查找指定任务对应的模块，并在找不到时抛出异常。
